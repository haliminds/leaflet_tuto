...
function initialize() {
  ...
  cats.forEach(onecat => {
    document.getElementById(onecat["id"]).addEventListener("click", handleCommand, false);
  });

  function handleCommand() {
    let selectedCat;
    // on recupere la categorie cliquee
    for (let onecat of cats) {
      if (onecat["id"] === this.id) {
        selectedCat = onecat;
        break;
      }
    }
...
function getCat(tabCat, category) {
  for (let oneTabCat of tabCat) {
    if (oneTabCat.label == category) {
      return oneTabCat;
    }
  }
  return undefined;
