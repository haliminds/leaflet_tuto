...
async function showPathToNearestCycleParkWithPos(lat, lon, lat_to = null, lon_to = null) {
  ...
  items.slice(-maxNbOfPark).forEach(function(parkingNode) {
    urlTabForCycleParkPath.push(OSRM_API + currentPos[1] + ',' + currentPos[0] + ';' + parkingNode[1].lon + ',' + parkingNode[1].lat + '?geometries=geojson');
  });
  ...
  Promise.all(urlTabForCycleParkPath.map(url =>
    ...
    .then(data => {
      ...
      data.sort((a, b) => a.routes[0].distance - b.routes[0].distance)
      const maxDist = data[0].routes[0].distance; // 10 km.
      const nearestPath = data[0].routes[0].geometry.coordinates.map(elem => [elem[1], elem[0]])
